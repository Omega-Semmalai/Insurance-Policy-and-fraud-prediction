...
user_input_reshaped = user_input_scaled.reshape((1, 1, n_features_to_select))

# Make prediction
user_pred_prob = model.predict(user_input_reshaped)
user_pred = (user_pred_prob > 0.5).astype(int)

# Display the prediction
print(f"Prediction probability of fraud: {user_pred_prob[0][0]:.4f}")
print(f"Fraud detected: {'Yes' if user_pred[0][0] == 1 else 'No'}")
